...
<x-navbarAdm />
    <div class="max-w-7xl mx-auto py-8">
        <h1 class="text-4xl font-bold text-center text-gray-800 mb-10">Listado de Usuarios</h1>

        <p class="text-center text-gray-600 mb-6 text-lg">
            Bienvenido al listado de usuarios. Aquí puedes ver la información, actualizar o eliminar registros.
        </p>
            ...
            <table class="min-w-full bg-white rounded-lg overflow-hidden shadow-lg">
                <thead class="text-white bg-gradient-to-r from-gray-700 via-gray-600 to-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-sm font-semibold tracking-wide uppercase">Correo</th>
                        <th class="px-6 py-4 text-left text-sm font-semibold tracking-wide uppercase">Contraseña</th>
                ...
                <tbody class="bg-gray-50 divide-y divide-gray-200">
                    @foreach ($user as $use)
                        <tr class="hover:bg-blue-50 transition-colors duration-300">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $use->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $use->emal }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $use->password }}</td>
                            <td class="px-6 py-4 whitespace-nowrap flex justify-center space-x-2">
                                <div>
                                    <button
                                        ...
                                        data-target="#modal{{ $use->id }}">
                                        Actualizar
                                    </button>
                                    @include('CRUD.actualizar') <!-- Modal de actualización -->
                                </div>
                                <div>
                                    <form action="{{ route('CRUD.destroy', $use->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
